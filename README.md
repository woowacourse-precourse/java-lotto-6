...
- ️10. 발행되는 각 로또와 유저가 선택한 로또가 겹치는 개수에 맞는 Rank를 지정한다
- 11. 랭크를 저장할 수 있는 enum map을 만든다.
- 12. 각 발행되는 로또에 맞는 랭크를 enum map에 저장한다
- 13. 부합하는 갯수와 가격을 구한 후 비율을 구한다
- 14. 계산한 값을 토대로 출력 값을 구한다
- 15. 각 값에서 예외처리를 발행시킬 수 있는 것을 출력한다
      - a) 구입 금액은 1000으로 나눠 떨어져야 한다
      - b) 숫자는 ,를 기준으로 구분한다- 안지켜졌을 경우 예외처리를 한다
      - c) 예외처리를 발생시키고 코드가 계속 진행 되게 한다
- 16. 예외 처리가 발생시 [ERROR]를 출력한다
- 17. rate 소수 둘째자리에서 반올림한다
- 18. 예외처리 시 예외처리 상황에 맞는 함수를 반환한다
