...
- [x] 보너스 번호를 요청한다
	...
	- [x] 사용자가 입력한 보너스 번호를 받는다.
		- [x] 잘못된 입력이 발생했을 경우(로또 번호 실패와 더불어, 이미 입력받은 당첨번호인지도 확인) 예외 처리한다.
- [x] 당첨 통계를 출력한다.
...
그렇다면 당첨 기준과 당첨금을 연결시킬 때 당첨금에 당첨기준을 제시하여 값을 가져올 것인가, 혹은 당첨확인기가 당첨금과 당첨기준을 둘 다 가진 채로 판단할 것인가?
당첨확인기가 당첨로또에 값을 전달해 일치하는 개수 및 보너스 일치 여부를 DTO로 건네받고, 이를 이용해 당첨기준을 확인하는 것까진 흐름에 큰 문제가 없을 것 같다.
당첨금에 당첨기준을 제시하는 것도 문제는 없으나, 당첨확인기가 스스로 판단하고 당첨금을 넘겨주는것도 괜찮지 않을까? 당첨금이 당첨기준을 알 필요도 없고 말이다.
그렇다면 당첨확인기는 당첨로또에게 해당 로또 티켓이 얼마나 일치하는지 묻고, 그 결과에 따라 스스로 판단하여 상금을 건네주는 형태이다.
혹은 당첨 기준을 당첨금에 제시하고 결과로 당첨금 객체를 받아온다..
아무리 생각해도 양쪽 다 명확하지 않았다. 이유가 뭘까.
당첨금이 갑자기 건네지는 것 자체가 이상했다.
오히려 당첨 기준이라는 것 자체가 건네받은 일치 개수, 보너스 일치 여부를 보고 스스로 판단하여 상금을 주는 것까지 포함이지 않을까?
그렇게 해야 당첨 확인기가 제어의 역할만 하고, 당첨 기준이 자료를 보고 판단한 후 금액을 건네는게 더 자연스러울 것 같다.
당첨 금액은 현실에서 자주 바뀌는 값인데, 당첨 기준까지 알고 있다면 사람들이 혼란스러워할 것 같다. 당첨 금액값을 바꾸면서 '다른 것까지 바꿔야 하지 않을까?'라는 고민을 하게 될 것 같다.
다만 당첨 기준이 스스로 판단하게끔 하면 책임의 범위가 좀 커진다. 현재 당첨 개수 -> 보너스 번호 체크 -> 당첨 금액 획득 순인데, 당첨 개수와 당첨 금액이 따로 enum으로 작성되어 있고 보너스 번호 체크는 당첨 금액에서 담당한다.
당첨 확인기 자체를 당첨 조건 확인 -> 금액 반환으로 가는 게 더 나을 것 같다.

그러나 위의 내용을 진행하다 보니, 오히려 다음과 같은 사항 때문에 하나의 enum에 `맞춘 개수, 보너스 체크 유무, 금액`을 모두 집어넣어야 하겠다고 생각했다. 기존과 같은 구조일 경우,
- 확장할 때 여러 코드를 건드려야 한다.
	- 로또 당첨 기준과 그에 대한 금액을 추가하고자 한다면(즉 확장이 일어나야 한다면), 당첨 기준에 가서 enum을 추가하고, 당첨금에 가서 enum을 추가하고, Map에 두 enum을 연결하여야 한다.
	- 만약 보너스 번호까지 고려해야 한다면 추가적인 메서드도 작성해줘야 한다. 
- 당첨 회수와 당첨금을 잘못 연결하여 오용 가능성이 생긴다.
	- Map에 연결을 시킬 때 잘못 작성한다면 원치 않는 결과가 나올 수 있다. 테스트 코드에서 확인할 수야 있으나, 너무 바쁜 일정에 추가적인 테스트를 작성하지 못 한다면 이는 큰 문제로 확산될 수 있다.
	- 보너스 번호인 경우에도 위와 같이 테스트코드에서 깜빡하고 확인하지 못 했다면 문제가 생긴다.
- 출력 결과를 위해 강제되는 코드들이 생긴다.
	- 금액만 출력하는 게 아닌, 당첨 기준과 금액을 모두 출력문구에 넣어줘야 한다. 강제로 짝을 지어 넣어줄 수는 있겠으나, 보너스 문구를 위해 DTO에 플래그를 심어야 한다. 만약 실수하여 플래그를 잘못 입력했을 경우 실제 조건과 동떨어진 출력문구가 나오게 된다.

따라서 하나의 enum에 모든 조건을 넣고, 해당 조건들을 잘 사용할 수 있도록 구조를 작성하여 쉬운 확장 및 오용 가능성을 줄이고, 출력할 때도 내부 enum 조건에 맞게 메시지를 자동으로 완성시키기로 했다.
즉, enum의 목적에 맞도록 손쉬운 확장을 고려하기로 했다.

유틸 클래스도, 싱글턴 클래스도 객체지향을 벗어난다면 어떻게 하는 게 제일 좋은 것인가
# 느낀 점
