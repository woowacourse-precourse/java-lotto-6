...
7. 당첨 번호를 입력 받는다.
   ...
   3. 쉼표(,)를 기준으로 6개의 숫자를 입력받습니다.
   4. 예외처리 : 입력 받은 값이 쉼표(,)를 기준으로 6개의 숫자가 아니면 IllegalArgumentException를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 그 부분부터 입력을 다시 받는다.
