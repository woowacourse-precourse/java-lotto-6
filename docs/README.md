...
### 1️⃣ 게임 처리 기능
- [x] 로또 번호의 범위는 1~45이다.
- [x] 로또 발행 시, 각 번호는 중복되지 않는 6개의 숫자로만 구성된다.
...
- [x] 로또 구입 금액을 입력받는다.
  - [x] 게임 플레이어가 입력한 금액만큼 구매되어 발행된 로또번호들을 출력한다.
...
- [x] 최종 당첨 내역과 수익률을 출력한 후, 게임이 종료된다.
...
- [x] 구매한 로또 갯수를 **출력**한다.
  - Format : `x개를 구매했습니다.`
- [x] 각 로또의 구성 숫자를 아래 형식으로 **출력**한다.
  - [x] 번호는 **오름차순**으로 정렬하여 보여준다. 
  - Format : `[x, x, x, x, x, x]`
...
- [x] 제공된 `Lotto` 클래스를 활용했는가
  - [x] `numbers` 의 접근 제어자는 수정하지 않았는가
  - [x] `Lotto` 클래스 필드를 추가하지 않았는가
