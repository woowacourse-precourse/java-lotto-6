...
## 기능 테스트 구현 목록
- [ ] 로또 번호가 오름차순으로 정렬되어 출력되는지 확인하는 테스트 메서드 구현  
- [x] pickUniqueNumbersInRange() 메서드가 번호의 범위가 1부터 45까지인지 확인하는 테스트 구현  
- [x] 로또 구입 금액이 1000원으로 나누어 떨어지지 않을 때 에러 메시지가 나오는지 확인하는 테스트 구현  
- [x] 로또 구입 금액이 숫자가 아닐 때 에러 메시지가 나오는지 확인하는 테스트 구현  
- [ ] 로또 티켓과 당첨 번호를 비교했을 때 올바르게 일치하는 번호 개수를 확인하는 테스트 구현  
