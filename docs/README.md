...
-[x] 로또의 '숫자'를 만드는 LottoNumberMaker 클래스
  - 주어진 Randoms 라이브러리를 이용하여, 1부터 45까지의 중복없이 정렬된 6개의 숫자 리스트를 반환
  - 입력이 "1,2,3,4,5,6"과 같은 문자열로 주어진 경우, 이를 숫자 리스트로 변환


-[x] 로또를 만드는 LottoMaker 클래스
  - 테스트를 위해, LottoNumberMaker를 주입받을 수 있도록 생성자 설정
  - 입력된 당첨 번호와 보너스 번호를 이용하여, WinningLotto 객체를 생성
    - 당첨 번호와 보너스 번호 각각에 대해 예외를 검증
  
  - 한 장의 로또를 만드는 기능을 별도로 정의
  
  - 입력된 금액을 이용하여, 금액 만큼의 로또 리스트 생성
    - 금액에 대해 예외를 검증하고, 한 장의 로또를 만드는 과정을 금액만큼 반복
  

-[x] 당첨 정보가 저장된 LottoPrizes enum
  - 각 상수는 당첨 조건과, 당첨금 정보를 저장하고 있음
  - 당첨금을 출력 형식대로 컴마로 구분하고 '원'을 붙여서 반환하는 기능 정의
  - 각 상수에 대한 갯수 정보를 저장하는 Map을 생성하는 기능 정의


-[x] 로또 당첨 결과를 처리하는 LottoResult 클래스
  - 각 당첨 경우에 대한 갯수를 저장하는 Map을 이용하여, 조건의 형식대로 출력하고 총 수익금을 얻어내는 기능 정의
...
## 예외 처리 사항
- 기본적으로, domain과 view의 분리를 위해 domain에서의 예외 검증은 LottoMaker 클래스에서, view에서의 예외 검증은 InputView에서 진행함
...
-[ ] LottoMaker에서의 검증
- 로또 구입 금액에 대한 검증
  - InputView에서는 문자열로 주어진 금액을 검증했다면, LottoMaker에선 정수로 주어진 금액을 검증
...
-[x] LottoMaker 클래스에 대한 테스트
  - 금액, 당첨 번호, 보너스 번호 각각에 대한 모든 예외를 검증함


-[x] LottoNumberMaker 클래스에 대한 테스트
...
-[x] LottoResult 클래스에 대한 테스트
  - 5개의 당첨 결과 중 1개만 당첨된 경우, 5개의 결과 모두 당첨된 경우를 이용하여 당첨금과 통계를 정확히 출력하는지 확인함.
