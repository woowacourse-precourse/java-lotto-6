...
- controller
    - GameController: 게임의 흐름을 잡고, 도메인 로직으로 부터 데이터를 받아 view로 전달합니다.
- domain
    - BonusNumber: 보너스 번호에 대한 도메인 객체입니다.
    - Lotto: 로또번호(List<Integer>)에 관한 도메인 객체입니다.
    - Pay: 사용자 지불 금액에 관한 도메인 객체입니다.
    - Game: 게임 진행에 관한 도메인 객체입니다.
- view
    - InputView : 입력을 담당합니다.
    - OutputView : 출력을 담당합니다.
- validator
    - LottoValidator : 로또번호에 관한 유효성 검증 함수집합입니다.
    - NumberValidator : 한자리 숫자에 관한 유효성 검증 함수집합입니다.
    - PayValidator : 사용자 지불금액에 관한 유효성 검증 함수집합입니다.
- util
    - Generator: 랜덤 번호를 생성하고, 새로운 List에 복사하여 반환(Randoms가 불변을 반환하기 때문)합니다.
    - Parser: InputView에서 받은 데이터를 도메인에 저장할 수 있도록 타입을 변환하는 역할입니다.
- constant
    - ConfigurationNumbers : 게임 진행에 관련된 enum 클래스입니다.
    - ExceptionMessage : 예외 처리에 메세지가 모인 enum 클래스입니다.
    - PrintMessage : 출력되는 메세지가 모인 enum 클래스입니다.
    - Rank : 로또 당첨 순위에 관한 정보를 담은 enum 클래스입니다.

# 💬 궁금해요

- 객체의 협력은 어떻게 구성하면 좋을지 같이 이야기 하면 좋을 것 같아요!