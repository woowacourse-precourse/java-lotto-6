...
웹 게임이라 생각하고, view를 브라우저에서 실행되는 클라이언트 사이드라 생각하고 유사하게 설계해보았습니다.

입력되는 문자열 값(구입 금액)은 클라이언트 단의 가벼운 테스트(InputValidator)를 거친 뒤 포장되어 서버(InputController)로 보내집니다.

서버(InputController)는 적절한 타입으로 변환하여 비즈니스 로직에 사용합니다.

Input/Output Controller가 View와 Service의 중간 다리 역할을 함으로써 view는 model로부터 한 가지 부분(에러 메세지 상수)을 제외하고 독립했습니다.

model과 view가 같은 에러메세지 상수를 공유하고 있는데 이를 제외한 부분 에서는 완전히 독립했다고 볼 수 있습니다.
