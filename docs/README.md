...
- [ ] 구입금액의 유효성을 체크한다.
    - [ ] 값이 null 일 경우 에러가 발생 한다.
    - [ ] 0 또는 음수일 경우 에러가 발생한다.
    - [ ] 구입 금액 단위가 틀릴 경우 에러를 발생 한다.
    - [ ] Integer.max 를 넘거 간다면 에러를 발생 한다.
- [ ] 숫자 집합(로또번호, 당첨번)의 유효성을 검사한다.
    - [ ] 값이 null 일 경우 에러가 발생 한다.
    - [ ] 개수가 다를 경우 에러를 발생 한다.
    - [ ] 중복이 발생할 경우 에러를 발생 한다.
    - [ ] 1~45 안의 숫자가 아닐 경우 에러를 발생 한다.
- [ ] 보너스 번호의 유효성을 검사한다.
    - [ ] 값이 null 일 경우 에러가 발생 한다.
    - [ ] 1~45 안의 숫자가 아닐 경우 에러를 발생 한다.
    - [ ] 당첨번호와 중복되 경우 에러를 발생 한다.
