...
6. 당첨 내역 계산 테스트
    ...
    - [x]  총 숫자가 6개 일치하면 1등 당첨 수를 +1 증가한다.
7. 당첨금 계산 테스트
   - [ ]  `해당 당첨 등수 * 해당 등수 당첨금`의 합을 계산한다.
   - [ ]  당첨금을 0원 이상 4294966000000000원 이하로 수령했다면 어떠한 Exception도 감지되지 않는다.
   - [ ]  당첨금을 0원 미만 4294966000000000원 초과로 수령했다면 IllegalArgumentException을 반환한다.