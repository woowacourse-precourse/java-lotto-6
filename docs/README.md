...
### `LottoSystem` 클래스에 몰려있는 주요 로직 분리

- [x] 로또 간의 숫자 비교 및 보너스 숫자 포함 여부 판단 로직을 `Lotto` 클래스로 분리한다.
- [ ] 랜덤한 우승 로또 생성 로직을 `LottoGenerator`라는 새로운 클래스로 분리한다.

### `LottoSystem` 및 `Player` 클래스의 역할 재정의

- [ ] `Player` 클래스 필드 중 purchaseAmount를 `LottoSystem`으로 이동한다.
- [ ] `LottoSystem` 클래스 필드 중 player 필드를 제거한다.
...
- [ ] `LottoSystem` 클래스 생성에 필요한 파라미터 중 1차 타입인 Integer를 PurchaseAmount의 2차 타입으로 변환한다.
