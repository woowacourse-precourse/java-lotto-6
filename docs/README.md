...
- [x] 에러 메시지를 출력한다.
- [x] 구매한 랜덤 로또 정보를 출력한다.
- [x] 당첨 내역을 출력한다.
- [x] 수익률을 형식에 맞추어 출력한다.

### 로또 프로그램의 내부 로직을 책임지는 Service 객체를 구현 -> LottoService

- [x] 투입 금액만큼 로또를 생성한다.
    - [x] 투입 금액을 1000으로 나누어서 그 만큼 로또를 생성한다.
    - [x] 로또의 생성은 Randoms.pickUniqueNumbersInRange(1, 45, 6)을 사용한다.
- [x] 로또 생성 이후 생성한 로또들의 번호들을 형식에 맞추어 반환한다. 
- [x] 사용자의 입력에 따른 당첨 로또를 생성한다. 
- [x] 최종 결과들을 반환한다.
    - [x] 당첨 개수들을 등수에 따라 정리하여 문자열 리스트 형태로 반환한다.
    - [x] 수익률을 Double 형태로 반환한다.

### view와 domain간 간접 연결을 하여 로또 프로그램을 전체적으로 진행하도록 구현한다. -> LottoController

- [x] view에서 받아온 값들을 검증하고 예외가 발생시 다시 view에 입력을 요청하도록 구현한다.
    - [x] 예외 발생시 catch 하여 [ERROR] 를 포함하는 메시지를 출력하도록 요청한다.
- [x] 로또의 금액을 입력받아 와서 그에 해당하는 만큼 로또를 생성하도록 요청 및 생성 결과를 출력한다.
- [x] 당첨 로또를 구성하는 숫자들과 보너스 숫자를 입력받아 와서 당첨 로또를 생성하도록 요청한다.
- [x] 당첨 결과와 수익률을 받아와서 출력하도록 요청한다.
...
- [x] 주어진 숫자가 해당 로또에 포함되어 있는지 확인한다.
- [x] 로또가 가진 숫자들에 따라 형식에 맞추어 문자열을 만들어 반환한다.

### 당첨 로또를 구현한다. -> WinningLotto

- [x] 당첨 로또를 생성한다.
    - [x] List 형태로 당첨 숫자들을 필드로 갖는다.
    - [x] 보너스 번호를 필드로 갖는다. 
- [x] 로또를 비교하여 로또 등수를 반환한다.

### 로또 등수를 구현한다. -> Rank

- [x] 당첨 로또와 겹치는 숫자 개수와 보너스 숫자에 따른 로또 등수를 생성한다.
- [x] 자신의 상금을 반환하는 기능 구현
- [x] Rank 내에서 동일한 숫자 개수와 보너스 번호가 있는 지 여부를 통해 Rank를 Matching 해주는 기능 구현
- [x] 자신의 Rank에 따른 결과 문자열을 형식에 맞춰 만들어서 반환하는 기능 구현 
...
- [x] 총 획득 금액을 계산한다.
- [x] 각 Rank에 따른 당첨 결과 문자열들을 오름차순으로 정렬하여 List 형태로 반환한다. 

### 당첨 내역을 제작해주는 클래스를 구현한다. -> ResultGenerator

- [x] 구매한 로또들과 당첨 로또들을 하나하나 대조하여 LottoResult를 만들어 내는 기능을 구현한다.
...
- [x] IllegalAlgumentException이 발생하는 상황을 모두 처리하며 [ERROR]로 시작하는 예외 메시지를 출력하자
    - [x] 숫자가 아닌 구입 금액 문자열을 입력하면 예외를 발생시킨다.
    - [x] 구입 금액 문자열이 음수로 입력되면 예외를 발생시킨다.
    - [x] `1,000`원 단위로 나누어 떨어지지 않는 구입 금액을 입력하면 예외를 발생시킨다.
    - [x] 로또 번호 입력이 숫자가 아닌 문자가 포함된 문자열이면 예외를 발생시킨다.
    - [x] 쉼표로 구분된 당첨 번호가 6개 아니면 예외를 발생시킨다.
    - [x] 로또 번호 입력이 정해진 범위`1 ~ 45`를 벗어나면 예외를 발생시킨다.
    - [x] 로또 번호 입력 간 중복된 숫자를 가진다면 예외를 발생시킨다.
    - [x] 보너스 번호가 숫자가 아닌 문자로 주어진 문자열이면 예외를 발생시킨다.
    - [x] 보너스 번호가 정해진 범위`1 ~ 45`를 벗어나면 예외를 발생시킨다.
    - [x] 보너스 번호가 당첨 번호와 중복된 숫자를 가진다면 예외를 발생시킨다.
- [x] 테스트가 정상적으로 실행되는가
- [x] 출력문을 정확히 구현하자
    - [x] 구입 금액 입력 시 입력 메시지를 출력한다.
    - [x] 금액에 알맞게 발행한 로또 수와 각 로또의 번호를 형식에 맞추어 출력한다.
    - [x] 당첨 번호 입력 시 입력 메시지를 출력한다.
    - [x] 보너스 번호 입력 시 입력 메시지를 출력한다.
    - [x] 로또 최종 결과 들을 형식에 맞추어 출력한다. (수익률은 소수점 둘째자리 반올림, 당첨 금액 오름차순 정렬)
- [x] 주어진 세팅을 모두 확인하자
    - [x] `./gradlew clean test` 명령이 정상적으로 실행되는가
    - [x] `java version`이 `17`인지 확인했는가
    ...
    - [x] `Lotto`에 필드(인스턴스 변수)를 추가하지 않았는가