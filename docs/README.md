...
## 🐣class UserInput: Input 받기
- constructor
- getPrice(): 로또 구입 금액을 입력 받는다.
...
- constructor
- createRandomNumber(): 로또 구입 개수만큼 랜덤 번호를 생성한다.
- sortNumber(): 로또 번호를 정렬한다.
`
## 🐥🐥class Lotto
- constructor
- validate(): 올바르게 번호를 입력했는지 확인

## 🐥🐥🐥class Judging
- constructor
- judging(): 로또 번호에서 일치하는 숫자 개수를 구한다.
- calculateRateOfReturn(): 수익률을 계산한다.

## 🐥🐥🐥🐥class Result: UI 관련 기능 구현
- constructor
- printResult(): 최종 당첨 결과를 프린트한다.
- printRateOfReturn(): 수익률을 프린트한다.
