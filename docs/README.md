...
- [x] 돈과 관련된 연산을 수행하는 Money 클래스
  ...
  - 필드는 long타입을 이용합니다. 
  - 사용되진 않으나, toString을 재정의하여 금액을 컴마로 구분하고 '원'을 붙여 문자열 형태로 반환하도록 설정하였습니다.


- [x] 한 장의 로또에 대한 정보를 저장하는 Lotto 클래스
  - 입력된 숫자에 대한 예외를 검증합니다.
    - 예외 메시지는 내부에 정의한 LottoErrorMessage라는 enum에 별도로 저장하였습니다.
  - 보너스볼 매칭 여부 파악을 위해, 한 개의 숫자를 포함하고 있는지 여부를 반환하는 기능을 정의하였습니다.
...
- [x] 입력을 받아들이는 InputView 클래스
  - 금액, 당첨 번호, 보너스 번호에 대한 입력을 받고 검증한 뒤 반환합니다.
...
- [x] View를 이용하여, 입출력을 관리하는 LottoViewController 클래스
  ...
  - 금액, 당첨번호, 보너스 번호의 입력 코드가 중복되는 문제는 제네릭을 이용하여 해결하였습니다.
...
## 예외 처리 사항
- 코드 중복을 감안하더라도, domain과 view의 분리를 위해 예외 검증은 domain, inputview에서 각각 진행합니다.
  - view에서는 문자열 형태를 주로 검증하고, domain에서는 숫자, 리스트로 파싱된 값을 위주로 예외를 검증합니다. 

- [x] InputView에서의 검증
  - 하나의 로또 번호를 검증하는 메서드를 별도로 정의하고, 당첨 번호와 보너스 번호를 검증할 때 이 메서드를 호출하여 번호에 대해 검증합니다.
...
## 테스트 구현 사항
- 상수 클래스에 대해선 테스트를 진행하지 않았음


- View에 대한 테스트는 진행하지 않음. 입력값 예외 발생은 LottoController에서 한번에 검증함
  - 마찬가지로, View만 이용하는 LottoViewController 클래스에 대한 테스트도 LottoController를 통해 수행
...
- [x] Money 클래스에 대한 테스트
  - 연산과 관련된 것은, 기본 연산이기에 테스트 하지 않았음
...
- [x] LottoGenerator 클래스에 대한 테스트
  ...
  - 정상적인 값을 넣었을 때에 정확한 로또를 반환하는지에 대한 테스트를 감안하여 프로덕션 코드를 구현하였으나, 이 부분은 의미가 없다고 생각하여 진행하지 않음.
      - 예외 검증만 통과한다면, 단순한 연산이기 때문임
...
- [x] LottoResult 클래스에 대한 테스트
  - 5개의 당첨 결과 중 1개만 당첨되는 모든 경우와 5개의 결과 모두 당첨된 경우를 이용하여 당첨금을 정확하게 반환하는지 테스트함
