...
- [x] 로또 구입 금액을 입력한다. - UI.getMoney()
  ...
  - [x] 1,000원으로 나누어 떨어지지 않는 경우 예외 처리한다.
    - [x] "[ERROR] 1000원으로 나누어 떨어지는 값을 입력해야 합니다." 메세지를 출력하고, 그 부분부터 입력을 다시 받는다. - validation.validateDivideBy1000()
- [x] 구입 금액 만큼 로또를 발행한다.
  - [x] 구입 금액으로 통해 발행해야하는 로또 개수를 확인한다.
  - [] 1개의 로또를 발행할 때 중복되지 않는 6개의 숫자를 뽑는다.
  - [x] 로또 번호는 오름차순으로 정렬하여 로또 객체에 저장한다.
