...
1. view
    1. inputView
       - [x] : 로또 구입 금액을 입력 받는다.
         ...
         - [x] : 정확하지 않은 값을 입력시 ERROR_NUMBER(-1)을 반환한다.
       - [x] : 당첨 번호를 입력 받는다.
         - [x] : 입력 값에 숫자만 들어온지 확인한다.
         - [x] : 입력 값이 로또 범위 내의 숫자인지 확인한다. 
         - [x] : 입력 값에 중복된 숫자가 있는지 확인한다.
         - [x] : 입력 값을 통해 반환된 List의 Size가 6인지 확인한다.
         - [x] : 정확하지 않은 값을 입력시 ERROR_NUMBER(-1)을 반환한다.
       - [x] : 보너스 번호를 입력 받는다.
         - [x] : 입력 받은 값이 숫자인지 확인한다.
         - [x] : 입력 받은 값이 로또 범위 내의 숫자인지 확인한다.
         - [x] : 입력 받은 값이 당첨번호와 중복되는지 확인한다.
         - [x] : 정확하지 않은 값을 입력시 ERROR_NUMBER(-1)을 반환한다.
