...
- [] 로또 구입 금액을 입력 받는다. - inputLottoPurchaseAmount()
  - [] (예외)로또 구입 금액이 1000원으로 나누어 떨어지지 않으면 예외처리를 한다.
- [] 입력한 금액에 대한 로또의 개수를 계산한다. - calculateAmountOfLotto()
- [] 로또를 발행한다.(개수만큼) - generateLotto()
- [] 당첨번호를 입력받는다. - inputWinningNumbers()
  - [] 번호는 쉼표(,)를 기준으로 구분한다.
  - [] (예외)쉼표가 5개가 아닐경우
...
- [] 발행한 로또 수량을 출력한다. - printQuantityOfLotto()
- [] 발행한 로또 번호를 출력한다. - printLottoNumbers()
  - [] 로또 번호는 오름차순으로 정렬한다.
- [] 입력한 로또 번호와 발행한 로또 번호를 비교한다. - compareLottoNumers()
- [] 당첨 내역을 출력한다. - printLottoWinningResult()
