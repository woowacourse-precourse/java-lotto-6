...
* 2차 유효성 검사까지 통과했을 경우 입력된 당첨번호를 List에 모두 저장 후 오름차순 정렬
...
#### 당첨 번호
* 입력된 6개의 당첨번호가 모두 유효하면 오름차순 정렬한 다음 당첨번호에 관한 로또 객체 인스턴스가 생성되는지를 테스트 
* 각각의 당첨 숫자마다 아래의 유효성 검사에 관한 테스트 수행
  * 문자열이 정수로 변환 가능하지 않으면 에러 메시지 출력
  * 입력받은 각각의 로또 번호가 1~45 사이 숫자가 아니면 에러 메시지 출력
...
* 보너스 번호가 다음의 유효성 검사를 통과하는지 확인
  * 입력한 보너스 번호를 양의 정수로 변환할 수 없으면 에러메시지 출력
  * 입력된 보너스 번호가 1~45 사이 정수가 아니면 에러메시지 출력
  * 보너스 번호가 6개의 로또 번호 중 한 개와 중복되면 에러메시지 출력

#### 당첨 번호와 발행받은 로또 번호 간 비교
* 현재 로또와 당첨번호 간 일치하는 숫자의 개수를 제대로 구했는지를 테스트
* 발행받은 현재 로또에 보너스 번호가 포함되어 있을 떄, 현재 로또가 2등이면 true, 그 외의 등수이면 false를 반환하는지 테스트
* 당첨된 로또의 개수가 각 등수별로 올바르게 산출되었는지를 테스트


### controller 패키지 관련
#### 전체 진행 과정
* 발행된 로또(들)에 대한 등수별 당첨 로또의 개수를 구한 결과가 올바른지 확인
* 수익률 계산 결과가 올바른지 확인
