...
이번에 테스트를 작성하며, 정적팩토리 메서드에서 of를 반환하는 LottoNumberCreator 클래스를 만들었는데, 이 클래스가 처음에 1회만 작동하기에 문제가 없다고 생각했는데, 테스트코드를 작성했더니 List의 길이를 초기화하지 않아서 of를 두번 반환하면 기존의 List+new List 이렇게 반환을 받았습니다.
이러한 것처럼 테스트코드를 통해 하나의 메서드라도 정상적으로 작동하는지, 또 합쳐졌을 때 잘 작동하는지 계속 구분하며 본인이 기억할 수 없기에, 테스트코드를 통해 데이터가 정확히 생성되는지 확인이 가능하단 것을 느꼈고, 
로또 입력같은 경우 이걸 일일이 계속 만들면서 체크할 수가 없기에, 몇등이 제대로 매핑이 되었는지, 등등 확인을 쉽게 할 수 있었습니다.
1등같은 경우 실행으로 테스트한다면 극악의 확률로 나올텐데, 이 확률에 기대고 계속 실행할 수는 없으니
테스트 코드를 통해서 한번에 매핑이 되는지 바로 확인이 가능하다는 것을 알게되었고, 이는 일처리의 비용이 많이 줄어들 것이라 생각이 들었습니다.
