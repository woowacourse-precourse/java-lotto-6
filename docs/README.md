...
- [x] 숫자를 넣으면 둘째자리에서 반올림한 값을 반환한다.

### LottoResultCalculator

- [x] 리스트를 비교하여 일치하는 값의 개수를 반환한다.

### WinningNumberInput

- [x] 문자열을 넣으면 쉼표를 기준으로 토큰화한다.
...
- [x] Java 코드 컨벤션 가이드를 준수했는가?
  - [x]함수/메서드가 한 가지 일만 하는가?
- [x] JUnit 5와 AssertJ를 이용 하여 테스트 코드를 통해 기능 동작을 하였는가?
- [x] Java Enum을 적용했는가?
- [x] 도메인 로직에 단위 테스트를 구현했는가? (UI: System.out, System.in, Scanner)는 제외
...
- [x] indent depth를 3이상 쓰지 않았는가?
- [x] 3항 연산자를 쓰지 않았는가?
- [x] 함수/메서드가 15라인을 넘지 않는가?
- [x] else 예약어를 쓰지 않았는가?
