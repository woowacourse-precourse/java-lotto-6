...
- [ ] PurchaseValidator를 구현한다.
    - [ ] 금액이 0보다 작거나 같은 경우는 예외 처리 한다.
    - [ ] 1000원으로 나누어 떨어지지 않는 경우 예외 처리 한다.
- [ ] LottoNumbersValidator를 구현한다.
    - [ ] 숫자의 개수가 6개가 아닌 경우는 예외 처리 한다.
    - [ ] 하나의 숫자라도 '(1 ~ 45)' 범위에서 벗어나면 예외 처리 한다.
    - [ ] 6개의 숫자들 간에 겹치는 숫자가 있다면 예외 처리 한다.
- [ ] BonusNumberValidator를 구현한다.
    - [ ] 숫자가 '(1 ~ 45)' 범위에서 벗어나면 예외 처리 한다.
    - [ ] 보너스 숫자가 당첨 숫자들 중에 하나와 겹치면 예외 처리 한다.
