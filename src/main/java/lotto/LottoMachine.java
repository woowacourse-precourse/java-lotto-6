...
public class LottoMachine {
    ...
    public List<Lotto> buyLottos() {
        ...
        var lottos = IntStream.of(lottoCount)
                .mapToObj((i) -> new Lotto(RandomUtil.createUniqueRandomListOf(1, 45, 6)))
    ...
    public void goLotto(List<Lotto> lottos) {
        ...
        match4 = lottos.stream().filter(lotto -> lotto.matchCount(winnings) == 5 && !lottos.matchBonus(bonus)).count();
        System.out.println("5개 일치 (1,500,000원) - " + match5 +"개");
        match5Bonus = lottos.stream().filter(lotto -> lotto.matchCount(winnings) == 5 && lottos.matchBonus(bonus)).count();
