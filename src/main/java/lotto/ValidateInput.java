...
public class ValidateInput {
	...
	public void checkLottoWinningNumbers(List<Integer> winningNumbers) {
		...
		for (int number : winningNumbers) {
			if (winningNumbers.size() != 6) {
				throw new IllegalArgumentException(errorMessage + "당첨 번호는 6자리입니다.");
			}
			if (number > 45 || number < 1) {
				throw new IllegalArgumentException(errorMessage + " 당첨 번호를 확인해주세요");
			}
			if (!numberSet.add(number)) {
				throw new IllegalArgumentException(errorMessage + " 당첨 번호에 중복된 값이 있습니다.");
			}
		}
	}
	...
	public int validateBonus(String inputBonus, List<Integer> winningNumbers) {
		...
		if (winningNumbers.contains(bonus)) {
			throw new IllegalArgumentException(errorMessage + " 보너스 숫자가 당첨 숫자와 중복됩니다.");
		}
		return bonus;
	}
