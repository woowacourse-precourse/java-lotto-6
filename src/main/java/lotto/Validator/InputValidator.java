...
public class InputValidator {
    ...
    public static boolean isEmpty(String input){
        if (input == null || input.isEmpty()) {
            throw new IllegalArgumentException(ERROR + NULL_OR_EMPTY_ERROR);
        }
        return true;
    }
    ...
    public static void checkDivisibleBy1000(String input) {
        if (Integer.parseInt(input) % 1000 != 0) {
            throw new IllegalArgumentException(ERROR + CAN_NOT_DIVIDE_BY_1000_ERROR);
        }
    }
    ...
    public static boolean isNumber(String input) {
        ...
        for (char c : input.toCharArray()) {
            if (!Character.isDigit(c)) {
                throw new IllegalArgumentException(ERROR + NOT_A_NUMBER_ERROR);
            }
        }
        return true;
    }
    ...
    public static boolean isValidRangeNumber(String input) {
        if(!input.contains(",")){
            if(Integer.parseInt(input) < 1 || Integer.parseInt(input) > 45){
                throw new IllegalArgumentException(ERROR + NOT_A_VALID_RANGE_NUMBER_ERROR);
            }
        }
    ...
    public static void checkCommaDelimiter(String input) {
        ...
        if (!matcher.matches()) {
            throw new IllegalArgumentException(ERROR + NOT_A_VALID_DELIMITER_ERROR);
        }
    }
    ...
    public static void checkDistinctNumbers(String input){
        ...
        for (String number : numbers) {
            if (!uniqueNumbers.add(number)) {
                throw new IllegalArgumentException(ERROR + NOT_A_DISTINCT_NUMBER_ERROR);
            }
        }
    }
