...
public class LottoWinningProfitCalculator {
	...
	public Double calculateLottoWinningProfit(LottoTicketList lottoTicketList,
		List<Integer> winningCaseCountList) {
		BigInteger sumOfWinningMoney = calculateSumOfWinningMoney(winningCaseCountList);
		Double purchaseMoneyValue = (double) (lottoTicketList.getLottoTicketList().size()
			* MONEY_UNIT.getValue());
	...
	private Double finalProfitRateValue(BigInteger sumOfWinningMoney, Double purchaseMoneyValue) {
		...
		return Math.round((sum / purchaseMoneyValue) * moneyUnit) / (double) moneyUnit
			* mathRoundSetter;
	}
	...
	private BigInteger calculateSumOfWinningMoney(List<Integer> winningCaseCountList) {
		...
		while (END_OF_COUNT.getValue() < counterForIterate) {
			...
			BigInteger winningMoney = new BigInteger(
				Integer.toString(lottoWinningCase.getWinningMoney()));
			BigInteger winningCaseCount = new BigInteger(
				Integer.toString(winningCaseCountList.get(counterForIterate)));
