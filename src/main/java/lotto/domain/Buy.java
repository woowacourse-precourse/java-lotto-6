...
public class Buy {
    ...
    public int getGames() {
        ...
        while (totalAmount < Constants.LOTTOPRICE) {
            totalAmount = exception.checkInt(Console.readLine(), message);
        }
        while (!correctAmount) {
            correctAmount = exception.checkAmount(totalAmount, correctAmount, message);
        }
        System.out.println();
