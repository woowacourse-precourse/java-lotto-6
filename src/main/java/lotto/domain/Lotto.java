...
public class Lotto {
    public static final String NUMBERS_SIZE_INVALID_EXCEPTION = "로또는 총 " + NumberService.LOTTO_SIZE + "개의 숫자로 구성되어야 합니다.";
    public static final String NUMBERS_DUPLICATE_EXCEPTION = "로또는 중복되지 않는 숫자로 구성되어야 합니다.";
    public static final String NUMBERS_RANGE_INVALID_EXCEPTION = "로또 번호는 " + NumberService.LOTTO_RANGE_MIN + "부터 " + NumberService.LOTTO_RANGE_MAX + " 사이의 숫자여야 합니다.";
    ...
    private void validateRange(List<Integer> numbers) {
        int validRangeNumber = (int) numbers.stream()
                .filter(n -> NumberService.LOTTO_RANGE_MIN <= n && n <= NumberService.LOTTO_RANGE_MAX)
                .count();
        if (validRangeNumber != NumberService.LOTTO_SIZE) {
