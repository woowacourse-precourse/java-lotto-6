...
public class LottoValidationHandler {
    public final static String INVALID_DUPLICATED_LOTTO_NUMBERS = ERROR.getMessage() + " 로또 번호에 중복된 숫자가 있습니다.";
    public final static String INVALID_LOTTO_NUMBERS_SIZE = ERROR.getMessage() + " 로또 번호가 6개가 아닙니다.";
    public final static String INVALID_LOTTO_NUMBERS_RANGE = ERROR.getMessage() + " 1 ~ 45 사이의 로또 번호가 아닙니다.";

    public static void validateDuplicatedNumbers(List<Integer> numbers) {
        Set<Integer> uniqueNumbers = new HashSet<>(numbers);
        if(!isValidLottoSize(uniqueNumbers.size())) {
            throw new IllegalArgumentException(INVALID_DUPLICATED_LOTTO_NUMBERS);
        }
    }
    ...
    public static void validationNumbersRange(List<Integer> numbers) {
        for(Integer number : numbers) {
            if(!isValidNumberRange(number)) {
                throw new IllegalArgumentException(INVALID_LOTTO_NUMBERS_RANGE);
            }
        }
    }
