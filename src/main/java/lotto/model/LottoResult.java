...
public enum LottoResult {
    ...
    public static LottoResult single(int countBall) {
        if(countBall == 3) return THREE_MATCHES;
        if(countBall == 4) return FOUR_MATCHES;
        if(countBall == 5) return FIVE_MATCHES;
        if(countBall == 6) return SIX_MATCHES;
        if(countBall == 7) return FIVE_PLUS_BONUS;
