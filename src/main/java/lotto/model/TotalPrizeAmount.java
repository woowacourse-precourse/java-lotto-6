...
public final class TotalPrizeAmount {
    ...
    private TotalPrizeAmount(long prizesAmount) {
        validateMinimumTotalPrizeAmount(prizesAmount);
        this.prizesAmount = prizesAmount;
    }

    private void validateMinimumTotalPrizeAmount(long prizesAmount) {
        if (prizesAmount < MIN_TOTAL_PRIZES_AMOUNT) {
            String exceptionMessage = String.format(TOTAL_PRIZES_AMOUNT_EXCEPTION_FORMAT, MIN_TOTAL_PRIZES_AMOUNT);
