...
public class InputLottoValidator {
    ...
    private static void validateEndsWithComma(final String lottoNumbers) {
        if (lottoNumbers.endsWith(DELIMITER)) {
            throw new InvalidLottoFormatException();
        }
    }

    private static void validateEmpty(final String lottoNumbers) {
        if (lottoNumbers.isEmpty()) {
            throw new EmptyException();
        }
    }

    private static void validateLottoLength(final String lottoNumbers) {
    ...
    private static void validateLottoIsNumeric(final String lottoNumbers) {
        try {
    ...
    private static void validateLottoNumberRange(final String lottoNumbers){
        String[] validatedInput = lottoNumbers.split(DELIMITER);
        IntStream.range(0, MAX_LOTTO_LENGTH)
