...
public class WinningNumValidator {
    ...
    public void isNull(String number) {
        if (number.equals("")) {
            throw new IllegalArgumentException(ERROR_MESSAGE + NOT_BLANK_ERROR_MESSAGE);
        }
    }
    ...
    public void isDivisionComma(String number) {
        for (char c : number.toCharArray()) {
            if (!(Character.isLetterOrDigit(c) || c == ',')) {
                throw new IllegalArgumentException(ERROR_MESSAGE + IS_DIVISION_COMMA_MESSAGE);
            }
        }
    }
    ...
    public void isSizeError(List<Integer> numbers) {
        if (numbers.size() != LOTTO_SIZE) {
            throw new IllegalArgumentException(ERROR_MESSAGE + SIZE_OVER_MESSAGE);
        }
    }
