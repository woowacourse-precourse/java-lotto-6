...
public class OutputView {
    ...
    public void printAllLottoNumbers(Integer buyAmount,List<Lotto> lottoList){
        System.out.println(buyAmount + HOW_MANY_BOUGHT_ANNOUNCE_MESSAGE);
    ...
    public void printStatistics(List<Integer> prizeList, Float percentage){
        System.out.println(STATISTICS_ANNOUNCE_INTRO_MESSAGE);
        //5,4,3,2,1등 result 출력
        for (int i = 0 ; i < RANK_TYPES; i++){
            System.out.println(RANK_MESSAGE_LIST.get(i) + prizeList.get(i) + UNIT_STRING + DOT);
        }
