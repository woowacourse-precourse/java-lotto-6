    ...
    public static void winningResult(HashMap<LottoRank, Integer> winningRankCount, double profitRate) {
        for (LottoRank lottoRank : LottoRank.values()) {
            if (lottoRank.equals(LottoRank.SECOND)) {
                System.out.printf(SECOND_WINNING_FORMAT,lottoRank.getRankMatchCount(),lottoRank.getPrizeAsString(),winningRankCount.getOrDefault(lottoRank,0));
                continue;
            }
            System.out.printf(WINNING_RESULT_FORMAT,lottoRank.getRankMatchCount(),lottoRank.getPrizeAsString(),winningRankCount.getOrDefault(lottoRank,0));
        }
