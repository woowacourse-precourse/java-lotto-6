...
public class OutputView {
    ...
    public void resultRateMessage(String rate) {
        System.out.print(OutputMessage.RATE_MESSAGE.getMessage() + Comma(rate) + OutputMessage.END_MESSAGE.getMessage());
    }

    private String Comma(String input) {
        return input.replaceAll("\\B(?=(\\d{3})+(?!\\d))", ",");
    }
}
