...
public class Validator {
    ...
    public static void checkLottoSizeNumberInput(List<Integer> numbers) {
        if (numbers.size() != 6) {
            OutputMessage.printError(ErrorType.INVALID_NUMBER_DIGITS.getText());
    ...
    public static void checkLottoDuplicatedNumberInput(List<Integer> numbers) {
        ...
        if (count != numbers.size()) {
            OutputMessage.printError(ErrorType.INVALID_NUMBER_DUPLICATED.getText());
    ...
    public static void checkLottoBonusNumber(List<Integer> numbers, int bonusNumber) {
        if (bonusNumber <= MIN_NUMBER || bonusNumber > MAX_NUMBER) {
            OutputMessage.printError(ErrorType.INVALID_BONUS_NUMBER_RANGE.getText());
            throw new NotValidBonusRangeException();
        }

        for (int number : numbers) {
            if (number == bonusNumber) {
                OutputMessage.printError(ErrorType.INVALID_BONUS_NUMBER_DUPLICATED.getText());
    ...
    public static void checkMoney(int money) {
        if (money % 1000 != MIN_NUMBER || money <= MIN_NUMBER) {
            OutputMessage.printError(ErrorType.INVALID_PRICE_INPUT.getText());
