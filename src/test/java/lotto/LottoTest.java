...
class LottoTest {
    ...
    class getResult_메서드는 {
        ...
        void 동일한_숫자가_존재하지않으면_NONE을_리턴한다() {
            ...
            Lotto lotto = new Lotto(List.of(7, 8, 9, 10, 11, 12));
            Assertions.assertThat(lotto.getScore(winningLotto)).isEqualTo(Score.NONE);
        }
        ...
        void 동일한_숫자가_3개_존재하면_FIFTH을_리턴한다() {
            winningLotto = new WinningLotto(winningNumbers, bonusNumber);
            Lotto lotto = new Lotto(List.of(7, 4, 9, 10, 5, 6));
            Assertions.assertThat(lotto.getScore(winningLotto)).isEqualTo(Score.FIFTH);
        }
        ...
        void 동일한_숫자가_4개_존재하면_FOURTH를_리턴한다() {
            winningLotto = new WinningLotto(winningNumbers, bonusNumber);
            Lotto lotto = new Lotto(List.of(3, 4, 9, 10, 5, 6));
            Assertions.assertThat(lotto.getScore(winningLotto)).isEqualTo(Score.FOURTH);
        }
        ...
        void 동일한_숫자가_5개_존재하면_THIRD를_리턴한다() {
            winningLotto = new WinningLotto(winningNumbers, bonusNumber);
            Lotto lotto = new Lotto(List.of(3, 4, 2, 10, 5, 6));
            Assertions.assertThat(lotto.getScore(winningLotto)).isEqualTo(Score.THIRD);
        }
        ...
        void 동일한_숫자가_5개_존재하고_보너스_숫자를_포함하면_SECOND를_리턴한다() {
            ...
            Assertions.assertThat(lotto.getScore(winningLotto)).isEqualTo(Score.SECOND);
        }
        @Test
        void 동일한_숫자가_6개_존재하면_FIRST을_리턴한다() {
            winningLotto = new WinningLotto(winningNumbers, bonusNumber);
            Lotto lotto = new Lotto(List.of(3, 4, 2, 1, 5, 6));
            Assertions.assertThat(lotto.getScore(winningLotto)).isEqualTo(Score.FIRST);
        }
    }
