...
public class LottoRankTest {
    ...
    void testLottoRankValueOf() {
        assertThat(LottoRank.valueOf(ZERO_MATCHES, NO_MATCH_BONUS)).isEqualTo(LottoRank.MISS);
    ...
    void testNoneRankCondition() {
        assertThat(LottoRank.valueOf(ONE_MATCH, NO_MATCH_BONUS)).isEqualTo(LottoRank.MISS);
        assertThat(LottoRank.valueOf(TWO_MATCHES, NO_MATCH_BONUS)).isEqualTo(LottoRank.MISS);
    }

    @Test
    @DisplayName("각 등수의 매치되는 숫자 개수를 올바르게 반환하는지 확인")
    void testGetMatchCount() {
        assertThat(LottoRank.FIRST.getMatch()).isEqualTo(SIX_MATCHES);
        assertThat(LottoRank.SECOND.getMatch()).isEqualTo(FIVE_MATCHES);
        assertThat(LottoRank.THIRD.getMatch()).isEqualTo(FIVE_MATCHES);
        assertThat(LottoRank.FOURTH.getMatch()).isEqualTo(FOUR_MATCHES);
        assertThat(LottoRank.FIFTH.getMatch()).isEqualTo(THREE_MATCHES);
    }

    @Test
    @DisplayName("보너스 번호 일치 여부를 올바르게 반환하는지 확인")
    void testIsMatchBonus() {
        assertThat(LottoRank.SECOND.getCheckBonus()).isEqualTo(MATCH_BONUS);
        assertThat(LottoRank.FIRST.getCheckBonus()).isEqualTo(NO_MATCH_BONUS);
        assertThat(LottoRank.THIRD.getCheckBonus()).isEqualTo(NO_MATCH_BONUS);
    }
