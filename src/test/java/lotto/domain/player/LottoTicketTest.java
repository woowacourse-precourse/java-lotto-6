...
public class LottoTicketTest {
     ...
     *                   10,000을 소비해 티켓 구매
     *                   구매할 Lotto 및 Lotto를 저장할 Lotties 생성
     */
    @BeforeEach
    void initData() {
    ...
    @DisplayName("lottoTicket 10장을 Lotto 로 바꿔 Lotties 에 넣으면, 티켓은 0장이 된다.")
    @Test
    void lottoTicketTest_2() {
        // given
        lottoTicket = lottoTicket.issueLottoTicket(playerWallet);
        // when
        lottoTicket = lottoTicket.changeAllTicketToLotto(boughtLotto, lotties);
        // then
        assertThat(lottoTicket.getLottoTicket()).isEqualTo(0);
    }

    @DisplayName("로또 구매후 lotties 를 lottoData 로 변환하면, lottoData 에는 로또 10개가 있으며 구매한 로또를 포함한다.")
    @Test
    void lottoTicketTest_3() {
        ...
        // when
        lottoTicket = lottoTicket.changeAllTicketToLotto(boughtLotto, lotties);
        List<Lotto> lottoData = lotties.makeLottoDto().getData();
        // then
