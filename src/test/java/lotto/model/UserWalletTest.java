...
class UserWalletTest {
    ...
    void buyLotto() {
        Assertions.assertEquals(userWallet.buyLotto(), MONEY);
    }

    @Test
    @DisplayName("createWallet_정적_팩토리_메서드_생성_테스트")
    void createWallet() {
        Assertions.assertEquals(userWallet.getMoney(), UserWallet.createWallet(MONEY).getMoney());
    }
    ...
    void receiveWinningsMoney() {
        ...
        Assertions.assertEquals(userWallet.getMoney(), 55000);
    }
}