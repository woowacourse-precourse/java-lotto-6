...
class AmountOfLottoValidatorTest {
  ...
  void integerSuccessTest(int amountOfLotto) {
    // given
    // when & then
    assertThatNoException().isThrownBy(() -> amountOfLottoValidator.checkPurchase(amountOfLotto));
  }
  ...
  void zeroFailTest(int amountOfLotto) {
    // given
    // when & then
    assertThatIllegalArgumentException().isThrownBy(() -> amountOfLottoValidator.checkPurchase(amountOfLotto));
  }
  ...
  void notDivideByLottoPrice(int amountOfLotto) {
    ...
    // when & then
    assertThatIllegalArgumentException().isThrownBy(
        () -> amountOfLottoValidator.checkPurchase(amountOfLotto));
  }
}