...
class LottoPurchaseListComparerTest {
    ...
    void testCompareUnitNumber() {
        ...
        assertEquals(3, result); // 1, 3, 5 세 개의 숫자가 일치
    }
    ...
    void testCompareBundle() {
        ...
        assertEquals(1, resultList.get(0)); // 3개 일치한 경우의 수 1개
        assertEquals(1, resultList.get(1)); // 4개 일치한 경우의 수 0개
        assertEquals(0, resultList.get(2)); // 5개 일치한 경우의 수 0개
        assertEquals(1, resultList.get(3)); // 6개 일치한 경우의 수 0개
        assertEquals(0, resultList.get(4)); // 7개 일치한 경우의 수 0개
    }
}
