...
public class LottoServiceTest {
    ...
    @ParameterizedTest
    @MethodSource
    void 사용자에게_지정되었던_로또와_당첨_번호_보너스_번호를_비교한다(List<Lotto> lottos, Lotto winngingLotto, int bonusNumber,
                                            LotteryResult expectResult, int expectCount) {
        //given
        Player player = Player.of(PurchasePrice.from(1000), lottos);
    ...
    private static Stream<Arguments> 사용자에게_지정되었던_로또와_당첨_번호_보너스_번호를_비교한다() {
        ...
        return Stream.of(
                Arguments.of(lottos, winningLotto1, 45, LotteryResult.FIRST, 1),
                Arguments.of(lottos, winningLotto2, 45, LotteryResult.FIFTH, 2),
                Arguments.of(lottos, winningLotto3, 18, LotteryResult.SECOND, 1)
        );
    }
