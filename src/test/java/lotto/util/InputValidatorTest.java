...
class InputValidatorTest {
    ...
    void validatePurchaseMoney() {
        assertDoesNotThrow(() -> InputValidator.validatePurchaseMoney(VALID_MONEY));
    }

    @Test
    @DisplayName("구입 금액이 숫자가 아닌 경우")
    void validatePurchaseMoneyNotNumber() {
        assertThrows(IllegalArgumentException.class, () -> InputValidator.validatePurchaseMoney(INVALID_MONEY_NOT_NUMBER));
    }

    @Test
    @DisplayName("구입 금액이 1000원 미만인 경우")
    void validatePurchaseMoneyMinimumLimit() {
        assertThrows(IllegalArgumentException.class, () -> InputValidator.validatePurchaseMoney(INVALID_MONEY_MINIMUM_LIMIT));
    }

    @Test
    @DisplayName("구입 금액이 1000원 단위가 아닌 경우")
    void validatePurchaseMoneyUnitLimit() {
        assertThrows(IllegalArgumentException.class, () -> InputValidator.validatePurchaseMoney(INVALID_MONEY_UNIT_LIMIT));
    }
}