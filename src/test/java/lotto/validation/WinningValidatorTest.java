...
public class WinningValidatorTest {
    ...
    void 당첨_번호_입력_값_범위_벗어난_경우_테스트(String winning) {
        assertThatThrownBy(() -> {
                    WinningValidator.winningIsCorrectRange(winning);
                }
        ).isInstanceOf(IllegalArgumentException.class)
                .hasMessage(ErrorProperty.NUMBER_IS_NOT_CORRECTLY_RANGE.toString());
    }
    ...
    void 당첨_번호_입력_값_개수_초과_또는_미만_검증_로직_테스트(String winningNumbers) {
        assertThatThrownBy(() -> {
                    WinningValidator.winningsCountIsOverOrUnder(winningNumbers);
                }
        ).isInstanceOf(IllegalArgumentException.class)
    ...
    void 당첨_번호_입력_값_양식_검증_로직_테스트(String winningNumbers) {
        assertThatThrownBy(() -> {
                    WinningValidator.winningsFormatIsCorrect(winningNumbers);
                }
        ).isInstanceOf(IllegalArgumentException.class)
