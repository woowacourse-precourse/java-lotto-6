...
class ExceptionHandlerTest extends IOTest {
    ...
    void validateBonusNumberIfOnlyNumber() {
        ...
        assertThrows(NumberFormatException.class, () -> {
            systemIn(number1);
            exceptionHandler.validateBonusNumber(winningNumbers);
        });
    }
    ...
    void validateWinningNumberNumberOrComma() {
        ...
        systemIn(winningNumber);
        assertThrows(IllegalArgumentException.class, exceptionHandler::validateWinningNumber);
    }
    ...
    void validateWinningNumberOnlySixNumbers() {
        ...
        systemIn(winningNumber);
        assertThrows(IllegalStateException.class, exceptionHandler::validateWinningNumber);
    }

    @Test
    @DisplayName("당첨 번호 입력 시 중복되는 숫자 입력할 시 예외 발생")
    void validateWinningNumberNoSameNumber() {
        String winningNumber = "1,1,3,4,5,6";
        systemIn(winningNumber);
        assertThrows(IllegalComponentStateException.class, exceptionHandler::validateWinningNumber);
    }
}